  ...
  Widget _buildContent() {
    return Column(
      ...
      children: [
        Stack(
          children: [
            MyNetworkImage.networkImage(70.r, user!.profilePhotoUrl ?? "", BoxFit.contain),
            Positioned(
              bottom: 0,
              right: 0,
              child: Container(
                ...
                child: Icon(Icons.camera_alt_outlined,
                    color: appTheme().colorScheme.lavender, size: 25.sp),
              ),
            ),
          ],
        ),
        Text(user!.fullName??" USER ", style: appTheme().textTheme.headlineSmall?.copyWith(color: appTheme().colorScheme.lavender)),
        SizedBox(height: 10.h),
        Text(user!.bio ?? "Lorem Ipsum is simply dummy text", style: appTheme().textTheme.titleMedium),
        SizedBox(height: 10.h),
        SecondaryButton(
          newHeight: 38.h,
          newWidth: 366.w,
          ...
          style: TextStyle(fontWeight: FontWeight.w500, fontSize: 15.sp, color: appTheme().colorScheme.lavender),
        ),
        SizedBox(height: 20.h),
        Expanded(child: GridView(gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2))),
      ],
    );
  }
}
